...
const createItems = () =>
  Array.from({ length: 100_000 }, (_) => createItem());
...
export function AyubDynamicHeight() {
  const [listItems, setListItems] = useState(createItems);
  const scrollElementRef = useRef<HTMLDivElement>(null);

  const { virtualItems, totalHeight, measureElement, startIndex, endIndex } = useDynamicSizeList({
  ...
  return (
    <div style={{ padding: "0 12px" }}>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <button
          onClick={() => setListItems((items) => [createItem(), ...items])}
        >
          add to the top
        </button>
      </div>
