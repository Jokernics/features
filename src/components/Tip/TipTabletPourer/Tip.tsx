...
const Tip = forwardRef<HTMLDivElement, tipPropsType>(function Tip(
  { children, tipContent, gapX = 0, gapY = 5, containerClassName = "", tipContainerClassName = "", manualOpen },
  containerRef
) {
  ...
  useLayoutEffect(() => {
    ...
    const arrowLeft = contentMetrics.left + contentRef.current.offsetWidth / 2 - arrowSize / 2 + gapX;
  ...
  const handleContentMouseEnter = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    if (manualOpen !== undefined || isMobile()) return;
        ...
        createPortal(
          <div className="contents">
            <div
              ...
              className={`border-indigo-800 bottom-1 w-fit h-fit flex flex-col items-center
                rounded-lg shadow-lg px-2 py-1 text-white text-sm
                border-2 bg-indigo-800 ${tipContainerClassName}`.trim()}
              ref={tipRef}
            ...
            <div
              ...
              className="w-0 h-0 border-[6px] border-solid border-transparent border-t-indigo-800"
            />
          </div>,
          document.body
        )}
